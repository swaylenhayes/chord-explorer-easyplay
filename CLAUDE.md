...
1. **The temperature palette is fixed; note-to-color mapping rotates with the selected key root.** The 12 hex values in `TEMPERATURE_PALETTE` never change. A note's color is determined by its circle-of-fifths distance from the current root key — position 0 (root) is always red (`#CC1F16`), position 11 (farthest) is always purple (`#6C2EAF`). Key changes rotate which notes occupy which color positions. In C major, the result is identical to the original fixed `NOTE_COLORS` map.
